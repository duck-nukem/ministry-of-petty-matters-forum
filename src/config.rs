...
pub static APP_CONFIG: LazyLock<Config> = LazyLock::new(|| {
    let public_root_url = std::env::var("PUBLIC_ROOT_URL").unwrap_or_else(|_| "http://localhost".to_string());
    let host = std::env::var("APPLICATION_HOST").unwrap_or_else(|_| "0.0.0.0".to_string());
